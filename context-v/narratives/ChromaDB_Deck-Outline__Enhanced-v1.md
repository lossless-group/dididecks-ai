...
lede: "The first enhanced variant of Chroma's Series A pitch — diverging from the Proto baseline with real customer logos, real team headshots, real pixel-art icons from Chroma, and whatever copy / layout / narrative moves emerge from the founder review. Currently a verbatim duplicate of Proto; begin diverging here."
...
deck_slug: pitch
deck_route: /scroll/pitch/enhanced-v1/   # TBD — route to be decided as Enhanced-v1 is authored
...
> **Status: Stub.** Currently a verbatim duplicate of [[ChromaDB_Deck-Outline__Proto]]. The body below still describes the Proto state. **Begin diverging here.** As Enhanced-v1 evolves, edit each slide's section to reflect what *changed* from Proto, what *stayed*, and *why*. Diffing this file against `ChromaDB_Deck-Outline__Proto.md` will show the running divergence.
>
> **First likely divergences** (mirrored from Proto's "What Enhanced-v1 might do" section, now this variant's working list):
>
> - Land the missing assets (real icons, real customer logos, real team headshots).
> - Decide whether to re-split slide 5 back into two slides.
> - Tighten slide 4 "How we win" copy.
> - Consider a richer slide 3 visual (sparklines vs. stat cards).
> - Consider adding a use-of-funds slide before the ask.
> - Re-author the closing as a forward CTA, not a bookend.
> - Decide on a default mode per audience.
>
> Once Enhanced-v1 has at least one real divergence from Proto, **bump `at_semantic_version` to 0.1.1.0**, update `date_authored_current_draft`, and move `status` from `Stub` → `Draft`.

## What "Enhanced-v1" means in our convention
